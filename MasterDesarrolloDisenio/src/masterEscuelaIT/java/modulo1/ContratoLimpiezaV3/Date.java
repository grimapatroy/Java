...
public class Date {
    ...
    public int caputureDay() {
        boolean error;
        do {
            int day= Console.getInstance().readInt("INGRESA DIA QUE SE VA A CANCELAR (1 al 30): ");
            error  = (day > 0 && day <= this.DAYS_PER_MONTH);
        }while ( !error );
        return day;
    }
    public int captureMonth() {
        boolean error ;
        do {
            int month = Console.getInstance().readInt("INGRESA MES QUE SE VA A CANCELAR(1 - 12): ");
            error = month <= this.MONTHS_PER_YEAR && month >= 1;
        }while(!error);
        return month;
    }
